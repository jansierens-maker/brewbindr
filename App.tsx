...
CREATE POLICY "Users can update own profile" ON profiles FOR UPDATE USING (auth.uid() = id) WITH CHECK (auth.uid() = id AND (SELECT role FROM profiles WHERE id = auth.uid()) = role);
